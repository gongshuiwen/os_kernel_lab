...
load_icode(int fd, int argc, char **kargv) {
     ...
     */
    cprintf("0x%08x argc:%d \n0x%08x argv0:%s 0x%08x\n", &argc, argc, kargv, kargv[0], kargv);
    ...
    //(3.1) read raw data content in file and resolve elfhdr
    struct elfhdr *elf;
    if ((elf = (struct elfhdr *)kmalloc(sizeof(struct elfhdr))) == NULL) {
        cprintf("elf null\n");
        ret = -E_INVAL_ELF;
    ...
    if (load_icode_read(fd, elf, sizeof(struct elfhdr), 0) != 0) {
        cprintf("elf load failed\n");
        ret = -E_INVAL_ELF;
        goto bad_elf_cleanup_pgdir;
    }

    cprintf("elf address: %0x e_magic: %0x\n",  elf, elf->e_magic);
    ...
    // (3.2) read raw data content in file and resolve proghdr based on info in elfhdr
    struct proghdr *ph;
    if ((ph = (struct proghdr *)kmalloc(sizeof(struct proghdr) * elf->e_phnum)) == NULL) {
        cprintf("ph null\n");
    };

    if (load_icode_read(fd, ph, sizeof(struct proghdr) * elf->e_phnum, elf->e_phoff) != 0) {
        cprintf("ph load failed\n");
    };
    ...
    for (; ph < ph_end; ph ++) {
        ...
        // cprintf("ph va: %0x file size:%d mem size:%d\n", ph->p_va, ph->p_filesz, ph->p_memsz);
    ...
    assert(pgdir_alloc_page(mm->pgdir, USTACKTOP, PTE_USER) != NULL);
    ...
    char *buffer = USERBASE;
    *(esp) = argc;
    cprintf("0x%08x argc:%d \n", esp, *esp);
    for (int i = 0; i < argc; i++) {
        copy_string(mm, buffer, kargv[i], EXEC_MAX_ARG_LEN + 1);
        *(++esp) = buffer;
        cprintf("0x%08x argv%d:%s 0x%08x\n", esp, i, *(esp), *(esp));
        buffer = buffer + EXEC_MAX_ARG_LEN + 1;
    }
    // char *argv[EXEC_MAX_ARG_NUM];
    // copy_kargv(mm, argc, *(esp + 1 + i), kargv);
...
do_execve(const char *name, int argc, const char **argv) {
    ...
    else if (!copy_string(mm, local_name, name, sizeof(local_name))) {
        unlock_mm(mm);
        return ret;
    }
...
#define __KERNEL_EXECVE(name, path, ...) ({                         \
    ...
    cprintf("kernel_execve: pid = %d, name = \"%s\", path = \"%s\".\n",    \
            current->pid, name, path);                            \
