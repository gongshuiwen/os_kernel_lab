...
sfs_dirent_read_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, int slot, struct sfs_disk_entry *entry) {
    ...
    // cprintf("file name: %s file ino: %d\n", entry->name, entry->ino);
    return 0;
}
...
sfs_lookup_once(struct sfs_fs *sfs, struct sfs_inode *sin, const char *name, struct inode **node_store, int *slot) {
    ...
    // cprintf("sfs_lookup_once done: %s\n", name);
    return ret;
}
...
sfs_io_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, void *buf, off_t offset, size_t *alenp, bool write) {
    // cprintf("sfs_io_nolock buf: %0x\n", buf);
    // cprintf("sfs_io_nolock inode: %d\n", sin->ino);
    struct sfs_disk_inode *din = sin->din;
    // cprintf("sfs_io_nolock size: %d\n", din->size);
    // cprintf("sfs_io_nolock blocks: %d\n", din->blocks);
    // cprintf("sfs_io_nolock direct0: %d\n\n", din->direct[0]);
    ...
    *alenp = 0;
    ...
    // cprintf("sfs_io_nolock offset: %d\n", offset);
    // cprintf("sfs_io_nolock endpos: %d\n", endpos);
    // cprintf("sfs_io_nolock blkno: %d\n", blkno);
    // cprintf("sfs_io_nolock nblks: %d\n\n", nblks);
  ...
  //LAB8:EXERCISE1 YOUR CODE HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
	 ...
	 *       NOTICE: useful function: sfs_bmap_load_nolock, sfs_buf`_op	
	*/
    
    if ((blkoff = offset % SFS_BLKSIZE) != 0) {
        size = (nblks != 0) ? (SFS_BLKSIZE - blkoff) : (endpos - offset);
        sfs_bmap_load_nolock(sfs, sin, blkno, &ino);
        // cprintf("sfs_io_nolock read buf1 blkoff: %d\n", blkoff);
        // cprintf("sfs_io_nolock read buf1 size: %d\n", size);
        // cprintf("sfs_io_nolock read buf1 ino: %d\n", ino);
    ...
    if (nblks) {
        sfs_bmap_load_nolock(sfs, sin, blkno, &ino);
        // cprintf("sfs_io_nolock read block nblks: %d\n", nblks);
        // cprintf("sfs_io_nolock read block ino: %d\n", ino);
    ...
    if ((blkoff = endpos % SFS_BLKSIZE) != 0) {
        sfs_bmap_load_nolock(sfs, sin, blkno, &ino);
        // cprintf("sfs_io_nolock read buf2 size: %d\n", blkoff);
        // cprintf("sfs_io_nolock read buf2 ino: %d\n", ino);
        sfs_buf_op(sfs, buf, blkoff, ino, 0);
        alen += blkoff;
    }
